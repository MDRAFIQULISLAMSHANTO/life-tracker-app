...
export default function IncomePage() {
  ...
  const incomeTx = useMemo(() => {
    return transactions
      .filter(
        (t) =>
          t.type === 'income' && t.date && String(t.date).slice(0, 7) === ledgerMonthKey
      )
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Income</h1>
          <p className="text-gray-500 mt-2 font-medium">
            Add income and manage income categories. Lists follow the ledger month <span className="font-semibold text-text-primary">{ledgerMonthKey}</span> (header).
          </p>
        </div>
        <div className="bg-background-card rounded-2xl p-4 shadow-soft">
          <div className="text-sm text-text-secondary">Total Income</div>
          <div className="text-2xl font-bold text-text-primary">{formatCurrency(totalIncome, currency)}</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] p-6 lg:p-10 shadow-sm border border-gray-100/50">
        ...
        <form
          onSubmit={(e) => {
            ...
            if (!result.ok) {
              setSubmitError(result.error || 'Failed to add income.')
              return
            }
            setAmount('')
            setDescription('')
            setCategory(incomeCategories[0] || 'Other')
          }}
      ...
      <div className="bg-white rounded-[2rem] p-6 lg:p-10 shadow-sm border border-gray-100/50">
        <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight mb-6">All Income</h2>

        {incomeTx.length === 0 ? (
          <div className="text-text-secondary">No income yet.</div>
        ) : (
              ...
              <tbody>
                {incomeTx.map((t) => (
                  <tr key={t.id} className="border-t border-gray-100 hover:bg-gray-50/40">
                    ...
                    <td className="py-3 px-2 font-semibold text-success whitespace-nowrap">
